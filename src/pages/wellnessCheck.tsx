...
export default function WellnessCheck() {
  ...
  const getMoodText = (val: number) => val >= 8 ? "Estou ótimo!" : val >= 5 ? "Tudo bem" : "Não estou muito bem";
  const getSleepText = (val: number) => val >= 8 ? "Dormi muito bem" : val >= 4 ? "Poderia ser melhor" : "Dormi mal";
            ...
            <div className="bg-white rounded-[24px] p-6 md:p-8 shadow-sm border border-[#F4E6EE] hover:shadow-md transition-shadow">
              <div className="flex flex-col md:flex-row gap-6 items-center">
                <div className="w-16 h-16 rounded-full flex items-center justify-center shrink-0 bg-[#FCE5F2]">
                  <img src="/imgs/wellnessCheck/smileIcon.svg" alt="Humor" className="w-8 h-8" />
                </div>
                <div className="flex-1 w-full flex flex-col gap-4">
                  <div className="flex justify-between items-end">
                    <div>
                      <p className="text-sm font-bold uppercase tracking-wide mb-1 text-[#E4007C]">Humor</p>
                      <h3 className="text-xl font-bold text-[#2D2D2D]">{getMoodText(mood)}</h3>
                    </div>
                    <div className="text-2xl font-black text-[#E4007C]">{mood}<span className="text-lg text-gray-400">/10</span></div>
                  </div>
                  ...
                  <div className="relative h-6 flex items-center slider-mood">
                    ...
                    `}</style>
                    <Slider value={[mood]} max={10} step={1} onValueChange={(val) => setMood(val[0])} className="cursor-pointer slider-mood" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-[24px] p-6 md:p-8 shadow-sm border border-[#F4E6EE] hover:shadow-md transition-shadow">
              <div className="flex flex-col md:flex-row gap-6 items-center">
                <div className="w-16 h-16 rounded-full flex items-center justify-center shrink-0 bg-[#FFF0E5]">
                  <img src="/imgs/wellnessCheck/moonIcon.svg" alt="Dormir" className="w-8 h-8" />
                </div>
                <div className="flex-1 w-full flex flex-col gap-4">
                  <div className="flex justify-between items-end">
                    <div>
                      <p className="text-sm font-bold uppercase tracking-wide mb-1 text-[#FB8235]">Dormir</p>
                      <h3 className="text-xl font-bold text-[#2D2D2D]">{getSleepText(sleep)}</h3>
                    </div>
                    <div className="text-2xl font-black text-[#FB8235]">{sleep}<span className="text-lg text-gray-400">/10</span></div>
                  </div>
                  ...
                  <div className="relative h-6 flex items-center slider-sleep">
                    ...
                    `}</style>
                    <Slider value={[sleep]} max={10} step={1} onValueChange={(val) => setSleep(val[0])} className="cursor-pointer slider-sleep" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-[24px] p-6 md:p-8 shadow-sm border border-[#F4E6EE] hover:shadow-md transition-shadow">
              <div className="flex flex-col md:flex-row gap-6 items-center">
                <div className="w-16 h-16 rounded-full flex items-center justify-center shrink-0 bg-[#E0EFF1]">
                  <img src="/imgs/wellnessCheck/runIcon.svg" alt="Atividade" className="w-8 h-8" />
                </div>
                <div className="flex-1 w-full flex flex-col gap-4">
                  <div className="flex justify-between items-end">
                    <div>
                      <p className="text-sm font-bold uppercase tracking-wide mb-1 text-[#005F6B]">Atividade</p>
                      <h3 className="text-xl font-bold text-[#2D2D2D]">{getActivityText(activity)}</h3>
                    </div>
                    <div className="text-2xl font-black text-[#005F6B]">{activity}<span className="text-lg text-gray-400">/10</span></div>
                  </div>
                  ...
                  <div className="relative h-6 flex items-center slider-activity">
                    ...
                    `}</style>
                    <Slider value={[activity]} max={10} step={1} onValueChange={(val) => setActivity(val[0])} className="cursor-pointer slider-activity" />
                  </div>
                </div>
              </div>
            </div>
